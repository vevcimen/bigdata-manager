...
public final class PropertyElf {
    ...
    private static void setProperty(Object target, String propName, Object propValue, List<Method> methods) {
        ...
        Method writeMethod = methods.stream().filter(m3 -> m3.getName().equals(methodName) && m3.getParameterCount() == 1).findFirst().orElse(null);
        if (writeMethod == null) {
            String methodName2 = "set" + propName.toUpperCase(Locale.ENGLISH);
            writeMethod = methods.stream().filter(m3 -> m3.getName().equals(methodName2) && m3.getParameterCount() == 1).findFirst().orElse(null);
        }
        if (writeMethod == null) {
