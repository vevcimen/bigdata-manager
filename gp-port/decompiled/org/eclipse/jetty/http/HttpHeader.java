...
public enum HttpHeader {
    ...
    private HttpHeader(String s2) {
        this(s2, false);
    }

    private HttpHeader(String s2, boolean pseudo) {
        this._string = s2;
        this._lowerCase = StringUtil.asciiToLowerCase(s2);
        this._bytes = StringUtil.getBytes(s2);
        this._bytesColonSpace = StringUtil.getBytes(s2 + ": ");
    ...
    static {
        CACHE = new ArrayTrie<HttpHeader>(630);
        for (HttpHeader header : HttpHeader.values()) {
            if (header == UNKNOWN || CACHE.put(header.toString(), header)) continue;
